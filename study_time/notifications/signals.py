...
def create_course_notification(sender, instance, created, **kwargs):
    if created:
        Notification.objects.create(
            title=f"New course: {instance.title[:100] + '...' if len(instance.title) > 100 else instance.title}",
...
def create_episode_notification(sender, instance, created, **kwargs):
    if created:
        Notification.objects.create(
            title=f"New episode: {instance.title[:100] + '...' if len(instance.title) > 100 else instance.title}",
...
def create_article_notification(sender, instance, created, **kwargs):
    if created:
        Notification.objects.create(
            title=f"New article: {instance.title[:100] + '...' if len(instance.title) > 100 else instance.title}",
